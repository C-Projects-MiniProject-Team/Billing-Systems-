...
namespace Billing_System.View
{
    public partial class frmSupplier : SampleView
    {
        ...
        {
            string qry = @"Select ROW_NUMBER() OVER(ORDER BY proID) AS 'Sr#', supID, sName 'Name', sPhone 'Phone', sEmail 'Email', sAddress 'Address'
                         from tblSupplier where cName like '%" + txtSearch.Text + "%' order by cusID";
